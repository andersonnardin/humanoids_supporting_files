"""Generate a rough RPO whole-body motion with a basic single-tree RRT.

The RPO reaches beneath a table while a single rapidly exploring random tree
searches a 12-dimensional posture vector: one coordinated squat variable, the
torso joint, five right-arm joints, and five left-arm joints. Every node and
interpolated edge must respect joint limits and avoid self-collision and table
collision, including the visible terminal hand mesh omitted by the RPO URDF's
collision model.
...
RPO_URDF = ROOT / "rpo_description" / "urdf" / "rpo.urdf"

STARTING_BASE_HEIGHT = 0.70
GROUND_CLEARANCE = 0.002
# The RPO foot mesh AABB extends below the surface used by PyBullet's
# narrow-phase distance query. This measured correction places that actual
# collision surface at GROUND_CLEARANCE above the plane.
FOOT_CONTACT_CORRECTION = 0.03724
SQUAT_HIP = -0.78
SQUAT_KNEE = 1.55
SQUAT_ANKLE = -0.77

TABLETOP_HEIGHT = 0.70
TARGET_POSITION = np.array((0.34, -0.16, 0.60))
...
RIGHT_HAND_NAME = "right_elbow_yaw_joint"
LEFT_FOOT_NAME = "left_ankle_roll_joint"
RIGHT_FOOT_NAME = "right_ankle_roll_joint"

# The terminal RPO link has a visual mesh but no active collision element.
HAND_LOCAL_MIN = np.array((-0.0058, -0.0315, -0.0315))
HAND_LOCAL_MAX = np.array((0.1383, 0.0315, 0.0315))
HAND_TARGET_LOCAL = np.array((0.100, 0.0, 0.0))
...
def apply_squat(robot: int, indices: dict[str, int], amount: float) -> None:
    ...
    for side in ("left", "right"):
        p.resetJointState(robot, indices[f"{side}_thigh_pitch_joint"], SQUAT_HIP * blend)
    ...
    corrected = (
        ...
        base_position[1],
        base_position[2] + GROUND_CLEARANCE - sole_z - FOOT_CONTACT_CORRECTION,
    )
    p.resetBasePositionAndOrientation(robot, corrected, (0.0, 0.0, 0.0, 1.0))


def load_rpo() -> int:
    if not RPO_URDF.exists():
        raise FileNotFoundError(f"RPO URDF not found: {RPO_URDF}")
    return p.loadURDF(
        str(RPO_URDF),
...
def create_table() -> list[int]:
    ...
    for x, y in ((0.36, -0.31), (0.36, 0.31), (0.88, -0.31), (0.88, 0.31)):
        parts.append(
            p.createMultiBody(
                baseMass=0.0,
...
def goal_state(
    ...
    nominal = seed.copy()
    for name, value in (
        ("left_arm_pitch_joint", -0.55),
        ("left_arm_roll_joint", 0.20),
        ("left_elbow_pitch_joint", 0.70),
    ):
    ...
    if best is None or best_error > settings.goal_tolerance:
        raise RuntimeError(
            f"No collision-free RPO reaching posture was found; best error: {best_error:.3f} m."
        )
    return best
...
def basic_rrt(
    ...
    raise RuntimeError(f"The RPO RRT failed after {settings.max_iterations} iterations.")
...
def generate_plan(seed: int) -> Plan:
    ...
    try:
        ...
        if not state_is_valid(robot, indices, active_joints, start, table_parts):
            raise RuntimeError("The initial RPO posture is in collision.")
        ...
        for state in trajectory:
            if not state_is_valid(robot, indices, active_joints, state, table_parts):
                raise RuntimeError("Final RPO trajectory validation detected a collision.")
...
def run(gui: bool = True, seed: int = 6) -> None:
    ...
    try:
        ...
        p.resetDebugVisualizerCamera(
            ...
            cameraYaw=62.0,
            cameraPitch=-17.0,
            cameraTargetPosition=(0.34, 0.0, 0.48),
        )
